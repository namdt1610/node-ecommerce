...
async function downloadAlternativeImages(): Promise<string[]> {
...
async function updateProductImages() {
    try {
        ...
        // Download all images
        const downloadPromises = iphoneImages.map((image) =>
        ...
        const successfulDownloads = iphoneImages.filter(
            (_, index) =>
                downloadResults[index].status === 'fulfilled' &&
                (downloadResults[index] as PromiseFulfilledResult<boolean>)
                    .value
        )
        ...
        console.log(
            `Successfully downloaded ${successfulDownloads.length} primary images`
        )
        console.log(
            `Successfully downloaded ${alternativeImages.length} alternative images`
        )
        ...
        for (const product of products) {
            ...
            const matchingImages = successfulDownloads.filter((img) =>
                product.name
                    .toLowerCase()
                    .includes(
                        img.productModel.toLowerCase().replace('iphone ', '')
                    )
            )
            ...
            if (matchingImages.length > 0) {
                ...
                imagesToUse = matchingImages.map(
            ...
            if (imagesToUse.length > 0) {
                ...
                console.log(
                    `Updated ${product.name} with ${imagesToUse.length} images`
                )
            }
        }
...
export { downloadImage, updateProductImages }
