...
import { getSocketService } from '../../modules/dashboard'
...
export function dashboardEventsMiddleware(eventData: DashboardEventData) {
        ...
        switch (eventData.type) {
            ...
            case 'payment':
                socketService.emitNotification({
                    ...
                    type:
                        eventData.action === 'created' ? 'success' : 'warning',
                    data: eventData.data,
                })
                break
        }
...
function getOrderMessage(action: string, data: any): string {
    switch (action) {
        case 'created':
...
function getUserMessage(action: string, data: any): string {
    switch (action) {
        case 'created':
...
function getProductMessage(action: string, data: any): string {
    switch (action) {
        case 'created':
...
function getPaymentMessage(action: string, data: any): string {
    switch (action) {
        case 'created':
