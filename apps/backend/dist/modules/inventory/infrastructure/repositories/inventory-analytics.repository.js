...
exports.InventoryAnalyticsRepository = void 0;
class InventoryAnalyticsRepository {
    ...
    async getTotalItems(warehouseId) {
        const where = { isActive: true };
        if (warehouseId) {
            where.warehouseId = warehouseId;
        }
    ...
    async getTotalQuantity(warehouseId) {
        const where = { isActive: true };
        if (warehouseId) {
            where.warehouseId = warehouseId;
        }
    ...
    async getLowStockCount(warehouseId) {
        const where = {
            isActive: true,
        ...
        return inventories.filter((inv) => inv.availableQuantity <= inv.lowStockThreshold).length;
    }
    async getOutOfStockCount(warehouseId) {
        const where = {
            isActive: true,
            availableQuantity: 0,
        };
    ...
    async getExpiredItemsCount(warehouseId) {
        const where = {
            isActive: true,
            expiryDate: {
                lt: new Date(),
    ...
    async getExpiringItemsCount(days = 30, warehouseId) {
    ...
    async getTopProductsByValue(limit = 10, warehouseId) {
        const where = { isActive: true };
        if (warehouseId) {
            where.warehouseId = warehouseId;
        }
    ...
    async getWarehouseStats(warehouseId) {
        const [totalItems, totalValue, totalQuantity, lowStockItems, outOfStockItems, expiredItems, expiringItems,] = await Promise.all([
...
exports.InventoryAnalyticsRepository = InventoryAnalyticsRepository;
