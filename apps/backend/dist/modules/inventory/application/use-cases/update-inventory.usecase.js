    ...
    async execute(id, updateData) {
        ...
        if (updateData.totalQuantity !== undefined) {
            if (updateData.totalQuantity < existingInventory.reservedQuantity) {
                throw new Error('Total quantity cannot be less than reserved quantity');
            }
        }
        if (updateData.lowStockThreshold !== undefined &&
            updateData.lowStockThreshold < 0) {
            throw new Error('Low stock threshold must be greater than or equal to 0');
        }
        ...
        // Update inventory
        const updatedInventory = await this.inventoryRepository.update(id, updateData);
